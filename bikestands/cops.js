...
function osbug_makeform() {
        ...
        calmType1.id = "bike-stand";
        calmType1.value = "bike-stand";
        calmType1.type = "radio";
        calmType1.name = "type";
        el2.appendChild(calmType1);
        var label = document.createElement("label");
        //label.for = "speedcam";
        label.appendChild(document.createTextNode(OpenLayers.i18n("Stand")));
        el2.appendChild(label);
        el2.appendChild(document.createElement("br"));
        var calmType2 = document.createElement("input");
        calmType2.id = "bike-wall-loops";
        calmType2.value = "bike-wall-loops";
        calmType2.type = "radio";
        calmType2.name = "type";
        el2.appendChild(calmType2);
        label = document.createElement("label");
        //label.for = "bump";
        label.appendChild(document.createTextNode(OpenLayers.i18n("Wall loops")));
        el2.appendChild(label);
        el2.appendChild(document.createElement("br"));
        var calmType3 = document.createElement("input");
        calmType3.id = "bike-butterfly";
        calmType3.value = "bike-butterfly";
        calmType3.type = "radio";
        calmType3.name = "type";
        el2.appendChild(calmType3);
        label = document.createElement("label");
        //label.for = "bump";
        label.appendChild(document.createTextNode(OpenLayers.i18n("Butterfly")));
        el2.appendChild(label);
        el2.appendChild(document.createElement("br"));
        var calmType3 = document.createElement("input");
        calmType3.id = "bike-parking-other";
        calmType3.value = "bike-parking-other";
        calmType3.type = "radio";
        calmType3.name = "type";
        el2.appendChild(calmType3);
        label = document.createElement("label");
        //label.for = "bump";
        label.appendChild(document.createTextNode(OpenLayers.i18n("Other")));
        el2.appendChild(label);
        el1.appendChild(el2);
        ...
        if (osb.osbLayer.usernameshort != null) {
            ...
            if (osb.osbLayer.username != null) {
                osb.osbLayer.usernameshort = osb.osbLayer.username.replace("@latlon.org/bikestands","");
            }
        }
        ...
        $("bike-stand").checked = true;
        $("osbuser").value = osb.osbLayer.usernameshort;
        $("saybtn").onclick = function() {
            var l = popup.lonlat;
            var t = "(error) ";
            if ($("bike-butterfly").checked) t = "(bike-butterfly) ";
            if ($("bike-wall-loops").checked) t = "(bike-wall-loops) ";
            if ($("bike-stand").checked) t = "(bike-stand) ";
            if ($("bike-parking-other").checked) t = "(bike-parking-other) ";
            l.transform(map.projection,map.displayProjection)
            /* if ($("osbuser").value == "NoName") {
                alert(OpenLayers.i18n("Please fill in your name"));
                return false;
            } */
            osb.osbLayer.setUserName($("osbuser").value + "@latlon.org/bikestands");
...
function putAJAXMarker(id, lon, lat, text, closed)
{
    ...
    if (closed) return;
    if ((text.indexOf("mulica") == -1) &&
        (text.indexOf("@latlon.org/bikestands") == -1)) return;
...
function init() {
    ...
    var date = new Date();
    cops = new OpenLayers.Layer.OSM("Cycle map", "http://c.tile.opencyclemap.org/cycle/${z}/${x}/${y}.png?" + date.getTime(), {numZoomLevels: 18, type: 'png'});
    ...
    map.addLayers([cops, mapnik, markers, osbLayer]);
    ...
    if (!map.getCenter()) {
        ...
        if (OpenLayers.Util.getParameters().mlon == null) {
            lonlat = new OpenLayers.LonLat(18.75331, 49.20985);
            zoom = 14;
        } else {
    ...
    sorry.innerHTML = OpenLayers.i18n("New stands can be added on zoom level 17 or greater");
    sorry.id = "sorry";
